...
class SystemOn(QMainWindow):
    ...
    def apply_graph_theme(self):
        ...
        graphs = [*self.cpu_graphs, self.memory_graph, self.sensor_gpu_temp_graph, self.sensor_gpu_power_graph]
    ...
    def rebuild_performance_graphs(self):
        ...
        if self.performance_mode == "overall":
            graph = RGraph(x_points=60, y_points=100, hue_offset=0, label="CPU Total")
        ...
        for i in range(count):
            graph = RGraph(x_points=60, y_points=100, hue_offset=0, label=f"{label_prefix} #{i}")
            graph.setFixedHeight(self.graph_height)
            graph.setMinimumWidth(1)
            graph.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
            self.cpu_graphs.append(graph)
    ...
    def update_process_table(self, process_data):
        ...
        if self.page_stack.currentIndex() != 1 and ui_signature == self.last_process_signature:
            return

        if ui_signature == self.last_process_signature and (current_ts - self.last_process_render_ts) < 0.9:
            return
    ...
    def update_graphs(self, cpu_usages, memory_usage):
        if self.performance_mode == "overall":
            ...
            if self.cpu_graphs:
                self.cpu_graphs[0].updateLatestDatapoint(total_usage)
    ...
    def update_sensors(
        ...
        self.sensor_gpu_temp_graph.updateLatestDatapoint(self._extract_numeric_prefix(gpu_temperature))
        self.sensor_gpu_power_graph.updateLatestDatapoint(self._extract_numeric_prefix(gpu_power_usage))
